...
public class Auth
{
    ...
    static bool tryLogin()
    {
        ...
        for (int j = 0; j < 10; j++)
        {
            ...
            {
                if(userLogin == Users[i] && userPassword == Passwords[i]){Console.Write("Zostały podane poprawne dane, możesz się zalogować"); isCorrectLogin = true; break;}
            }
            if(!isCorrectLogin)Console.Write("Podanie dane są niepropawne");
            Thread.Sleep(1500);
            Console.Clear();
    ...
    static bool Logalkolator()
    {
        ...
        using (StreamReader sr = new StreamReader(path + "examples.txt"))
        {
            ...
            while (!sr.EndOfStream){string line = sr.ReadLine()??"";i++;Examp.Add(line);}
            exercise = Examp[random.Next(0, i)];
        }
        Console.Write(exercise + " = ");
        string userAnswer = Console.ReadLine() ?? "";
        string correctAnswer = Convert.ToString(Calc.CalculateRPN(Calc.ConvertToRPN(Calc.expressionToList(exercise))), CultureInfo.CurrentCulture);
        Console.WriteLine($"Poprawna odpowiedź: {correctAnswer}\nTwoja odpowiedź: {userAnswer}");
        if (userAnswer == correctAnswer) { Console.Write("Przyznano dostęp"); isSober = true; }
        else Console.Write("Odmówiono dostępu");
        Thread.Sleep(1500);
        Console.Clear();
        return isSober;
    }
